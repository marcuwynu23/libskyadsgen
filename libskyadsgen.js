...
const handleApiResponse = async () => {
	try {
		...
		const response = await fetch(
			"http://localhost:7575/api/cloud/cams/adsEngine"
		);
		if (!response.ok) {
		...
		// Iterate through the list of advertisements and display them sequentially

		if (adContent.type === "video") {
			modal.open(createVideoAdsContainer(adContent.content), 2);
		} else if (adContent.type === "image") {
			modal.open(createSlideShowAdsContainer(adContent.content), 5);
